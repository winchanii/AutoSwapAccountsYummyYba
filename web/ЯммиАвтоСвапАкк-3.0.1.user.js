...
(function() {
    ...
    const SOURCE_RAM_SETTINGS = {
        host: "your_lan_api_or_localhost",
        port: yor_port,
        password: "u_pass"
    };
    ...
    const RESTART_SERVER_SETTINGS = {
        host: "ip",
        port: 8080  // Порт для /restart (новый сервер)
    };
    ...
    const BATCH_TRIGGER_COUNT = kolvo_accov; // Количество "полных" аккаунтов для срабатывания отправки

    const DEBUG_MODE = true; //если хотите чтобы все действия комментировались в консоль браузре (f12)
    // --- Конец настроек ---
    ...
    /**
     * Выполняет последовательность действий после перезапуска webrb:
     * 1. Ждет 10 секунд после перезапуска
     * 2. Отправляет /trigger на порт 5000
     */
    async function performPostRestartActions() {
        debugLog("🚀 Начало выполнения действий после перезапуска webrb");
        try {
            // 1. Подождать 10 секунд
            debugLog("⏳ Ожидание 10 секунд после перезапуска webrb...");
            ...
            debugLog("🏁 Все действия после перезапуска webrb выполнены успешно");
        } catch (error) {
            console.error("[POST RESTART ACTIONS] Ошибка при выполнении действий:", error);
            debugLog(`❌ Ошибка в performPostRestartActions: ${error.message}`);
            // Не останавливаем основной поток, продолжаем проверки
        }
    }

    /**
     * Выполняет одно действие: отправляет /restart на порт 8080
     * Затем выполняет post-actions (ожидание и триггер)
     */
    async function performRestartAction() {
        debugLog("🚀 Начало выполнения действия перезапуска");
        try {
            // Отправить /restart на порт 8080 (новый сервер)
            const restartUrl = `http://${RESTART_SERVER_SETTINGS.host}:${RESTART_SERVER_SETTINGS.port}/restart`;
            await sendGetRequest(restartUrl);
            debugLog("✅ Команда /restart отправлена успешно");

            // Выполняем действия после перезапуска
            await performPostRestartActions();
            debugLog("✅ Выполнены действия после перезапуска webrb.");

            debugLog("🏁 Действие перезапуска и post-actions выполнены успешно");
        } catch (error) {
            console.error("[RESTART ACTION] Ошибка при выполнении перезапуска:", error);
            debugLog(`❌ Ошибка в performRestartAction: ${error.message}`);
            // Не останавливаем основной поток, продолжаем проверки
        }
    }
            ...
            if (fullAccounts.size >= BATCH_TRIGGER_COUNT) {
                ...
                if (accountsToSend.length >= BATCH_TRIGGER_COUNT) {
                    ...
                    if (sentCount === BATCH_TRIGGER_COUNT) {
                        await performRestartAction(); // Перезапуск + post-actions
                        debugLog("✅ Выполнено действие перезапуска и post-actions.");
                    } else {
                        debugLog(`⚠️ Не все куки были отправлены успешно (${sentCount}/${BATCH_TRIGGER_COUNT}), пропускаем перезапуск.`);
                    }
                    // --- КОНЕЦ ИЗМЕНЕНИЙ ---
    ...
    function init() {
        console.log("[INIT] YummyAuto Money Tracker & Account Switch Notifier v3.0.1 (Restart + Trigger)");
        debugLog("🔧 Инициализация скрипта");
        debugLog("⚙️ Конфигурация:", {
            SOURCE_RAM_SETTINGS,
            TARGET_SERVER_SETTINGS,   // Порт 8081 для кук
            RESTART_SERVER_SETTINGS,  // Порт 8080 для /restart (новый сервер)
